...
| | 做法 | 代價 | 適合 |
|---|---|---|---|
| **A** | **守 NES，改用量。**54 色硬約束不動，但大面積改用灰階格（`#000000`/`#787878`/`#BCBCBC`/`#FCFCFC`）與 `#503000`，高彩度只留給小面積強調。這是 8-bit 遊戲 UI 真正的做法 —— 背景吃暗色，亮色留給角色與提示。 | 梗圖卡不再是整片綠，品牌辨識度下降 | 想保住「硬體真實」這個賣點 |
| **B** | **分層放寬。**角色 sprite 繼續守 NES 54 色（硬體真實感留在角色上），UI 表面改用可自由調彩度／明度的色盤。 | 「全部都是 NES」這個宣稱不再成立，`design/hwpalette.mjs` 的單一真相要拆成兩組 | 想要好看優先，角色仍是 8-bit |
| **C** | **只留幾何。**放棄 8-bit 配色，保留方角、硬陰影、像素字重、dashed 分隔線。 | 8-bit 感只剩形狀 | 覺得整個 8-bit 色系方向就是錯的 |

**建議：A**。理由是第 ① ② ③ 條成因都指向「用量」而不是「色表」—— NES 遊戲本身並不醜，醜的是把 sprite 用色邏輯套到大面積 UI 上。選 A 保留既有投資，而且有明確的執行原則可依循。但這是品味決策，Cross 說了算。
...
| `tests/e2e/system.spec.ts` | 兩條硬編色斷言 | **這是 DEV-PLAN 明訂的唯一「可以改斷言」例外**。改的理由必須是「正確答案變了」，不是「讓測試變綠」，而且要在註解裡寫清楚 |
| 階段 3b 的 28 項 WCAG 檢查 | 已通過 | 換色盤後**全部失效，必須重跑** |
...
1. 全站（8 個畫面 × 淺／深兩種模式）沒有任何色值落在選定色盤之外 —— 用瀏覽器量 `getComputedStyle` 的渲染後實際值，不是讀 CSS 原始碼
2. WCAG AA 對比全過（正文 4.5:1、大字 3:1），淺深兩套各自驗
3. 三段火力仍然分得出來（階段 2 已把梯度做在幾何上：邊框 1/2/3px、硬陰影 2/3/4px、字重 750/850/950，色只是第四個維度）
4. 高對比模式（`prefers-contrast:more`）不會讓任何一層的邊框**變細**（見 2.3 的坑）
5. `npm run e2e` 全綠
...
目前三個角色全都是士官。Cross 提到「不同等級的軍官」，代表可能想擴充。有一個現成的鉤子：

`src/rules.ts:31-32` 與 `src/prompt.ts:25`（絕對規則第 5 條）規定，遇到大事一律回「去找連長。」—— **連長已經在規格裡，是大事的升級對象，但沒有角色、沒有立繪、沒有語料。** 連長是上尉，正好是軍官。

| 選項 | 範圍 |
|---|---|
| **C-1** 只修徽章 | `Level 0/1/2` 三個士官角色各自畫對的士官階級章。不加角色。範圍小，這一輪做得完 |
| **C-2** 加連長 | 在 C-1 之上，把連長做成第四個角色（大事畫面出現）。要新增語料庫 ≥20 句、5 張 sprite（idle/bark/praise/punish + 一張大事專用）、`Level` 型別要擴充 |
| **C-3** 完整階級體系 | 士官 + 尉 + 校 + 將全套。**不建議** —— 決斷連的核心是「班長給你一道命令」，將官在這個敘事裡沒有位置，會稀釋產品 |

**建議：C-1 先做，C-2 當獨立一輪。** 理由：C-1 是修正既有錯誤（三層共用假徽章），風險可控、驗收明確；C-2 是新功能，會動到 `Level` 型別、語料庫、五張新資產與大事流程，兩件混在一起做會失控。

> ⚠️ 型別警告：`Level = 0 | 1 | 2` 被大量使用（`officers.ts`、`avatar.tsx`、`prompt.ts`、`types.ts`、`app.tsx`、`design/*.mjs`）。C-2 要擴充它之前，先全域盤點所有消費點。
...
| 4 | **A 色盤** | 要 Cross 拍板才能動，且會讓階段 3b 的驗證全部作廢、要重跑 | Cross 選 A/B/C |
| 5 | **C 階級徽章** | 要先查資料，且徽章色值取決於階段 A 的結果 | 階段 A 定案 + C-3 查證 |
...
| **階段 A 三選一** | 見 A-2 |
| **階段 C 範圍決策** | C-1 只修徽章 / C-2 加連長，見 C-4 |
