...
> **狀態：Cross 已於 2026-09-17 拍板「選項 A」——守 NES 硬體色盤約束，改變用量。**
...
> 這組是**候選不是定案** —— 要在瀏覽器裡量渲染後實際值、確認每一格對黑底與白底都過 3:1，才能寫進 `styles.css`。
...
| **看梗圖卡新樣子的預覽** | 選項 A 推導出梗圖卡從綠底變中性底（A-2.4）。這是整輪視覺衝擊最大的一步，開發端會先做預覽給你看過再全面套用 |
